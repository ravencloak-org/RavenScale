...
func NewState(cfg *types.Config) (*State, error) {
	...
	ipAlloc, err := hsdb.NewIPAllocator(hsdb.WithAllTenants(context.Background()), db, cfg.PrefixV4, cfg.PrefixV6, cfg.IPAllocation)
	if err != nil {
	...
	pol, err := hsdb.PolicyBytes(db.DB.WithContext(hsdb.DefaultTenantCtx()), cfg)
	if err != nil {
...
func (s *State) ReloadPolicy() ([]change.Change, error) {
	pol, err := hsdb.PolicyBytes(s.db.DB.WithContext(hsdb.DefaultTenantCtx()), s.cfg)
	if err != nil {
		return nil, fmt.Errorf("loading policy: %w", err)
	}
...
func (s *State) CreateUser(user types.User) (*types.User, change.Change, error) {
	...
	if err := s.db.DB.WithContext(hsdb.DefaultTenantCtx()).Save(&user).Error; err != nil { //nolint:noinlineerr
...
func (s *State) UpdateUser(userID types.UserID, updateFn func(*types.User) error) (*types.User, change.Change, error) {
	user, err := hsdb.Write(hsdb.DefaultTenantCtx(), s.db.DB, func(tx *gorm.DB) (*types.User, error) {
...
func (s *State) DeleteUser(userID types.UserID) (change.Change, error) {
	err := s.db.DestroyUser(hsdb.DefaultTenantCtx(), userID)
	if err != nil {
...
func (s *State) GetUserByOIDCIdentifier(id string) (*types.User, error) {
	return s.db.GetUserByOIDCIdentifier(hsdb.DefaultTenantCtx(), id)
}
...
func (s *State) persistNodeRowToDB(node types.NodeView) (types.NodeView, error) {
	...
	err := s.db.DB.WithContext(hsdb.DefaultTenantCtx()).Select(nodeUpdateColumns).Omit("Expiry").Updates(nodePtr).Error
	s.persistMu.Unlock()
...
func (s *State) DeleteNode(node types.NodeView) (change.Change, error) {
	...
	err := s.db.DeleteNode(hsdb.DefaultTenantCtx(), node.AsStruct())
	if err != nil {
		return change.Change{}, err
	}
...
func (s *State) SetNodeExpiry(nodeID types.NodeID, expiry *time.Time) (types.NodeView, change.Change, error) {
	...
	err := s.db.NodeSetExpiry(hsdb.DefaultTenantCtx(), nodeID, expiry)
	if err != nil {
...
func (s *State) BackfillNodeIPs() ([]string, error) {
	changes, err := s.db.BackfillNodeIPs(hsdb.DefaultTenantCtx(), s.ipAlloc)
	if err != nil {
		return nil, err
	}
...
func (s *State) CreatePreAuthKey(userID *types.UserID, reusable bool, ephemeral bool, expiration *time.Time, aclTags []string) (*types.PreAuthKeyNew, error) {
	return s.db.CreatePreAuthKey(hsdb.DefaultTenantCtx(), userID, reusable, ephemeral, expiration, aclTags)
}
...
func (s *State) applyAuthNodeUpdate(params authNodeUpdateParams) (types.NodeView, error) {
	...
	_, err := hsdb.Write(hsdb.DefaultTenantCtx(), s.db.DB, func(tx *gorm.DB) (*types.Node, error) {
...
func (s *State) createAndSaveNewNode(params newNodeParams) (types.NodeView, error) {
	...
	savedNode, err := hsdb.Write(hsdb.DefaultTenantCtx(), s.db.DB, func(tx *gorm.DB) (*types.Node, error) {
